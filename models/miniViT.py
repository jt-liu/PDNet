...
class PatchTransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('layers.py 24',x.shape)#(300,2,128)
        return x
...
class PixelWiseDotProduct(nn.Module):
    ...
    def forward(self, x, K):
        n, c, h, w = x.size()
        _, cout, ck = K.size()
        # print('layers.py 35',K.size())#(2,128,128)
        assert c == ck, "Number of channels in x and Embedding dimension (at dim 2) of K matrix must match"
        y = torch.matmul(x.view(n, c, h * w).permute(0, 2, 1), K.permute(0, 2, 1))  # .shape = n, hw, cout
        return y.permute(0, 2, 1).view(n, cout, h, w)

class mViT(nn.Module):
    def __init__(self, in_channels, patch_size=16, n_query_channels=256, embedding_dim=4, num_heads=4):
        ...
        # self.project = nn.Conv2d(embedding_dim, embedding_dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # range_attention_maps = self.project(range_attention_maps)
        # range_attention_weight = torch.sigmoid(range_attention_maps)
